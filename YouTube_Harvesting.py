...
#API key connection
def Api_connect():
    Api_Id="AIzaSyDXfbAGlShMapgC1vpcQo3prKefHq8cvgQ"
...
#get playlist ids from youtube API
...
#get video ids from youtube API
...
#get comment information from youtube API
...
#client = pymongo.MongoClient("mongodb://kulkarnisupriya087:Astor@2620@ac-ek43if6-shard-00-00.dflc5in.mongodb.net:27017,ac-ek43if6-shard-00-01.dflc5in.mongodb.net:27017,ac-ek43if6-shard-00-02.dflc5in.mongodb.net:27017/?ssl=true&replicaSet=atlas-zj7o42-shard-0&authSource=admin&retryWrites=true&w=majority")
db = client["Youtube_data"]

# upload the data from API to MongoDB
...
def videos_table():
    ...
    for index, row in df2.iterrows():
        ...
        try:    
            ...
            st.write("Video values already inserted in the table")
...
with st.sidebar:
    ...
    st.caption("Data Storage using MongoDB and SQL")
    
channel_id = st.text_input("Enter the Channel id")
channels = channel_id.split(',')
channels = [ch.strip() for ch in channels if ch]

if st.button("Collect data and store in MongoDB database"):
...
if st.button("Migrate the data from MongoDB to SQL"):
    display = tables()
    st.success(display)
    
show_table = st.radio("SELECT THE TABLE FOR VIEW",(":Yellow[Channels]",":Red[Playlists]",":Blue[Videos]",":Orange[Comments]"))

if show_table == ":Yellow[Channels]":
    show_channels_table()
...
question = st.selectbox(
    ...
    ('1. All the videos and the Channel Name',
     ...
     '4. Comments made on each video, and what are their corresponding video names?',
...
if question == '2. Channels with most number of videos':
...
elif question == '4. Comments made on each video, and what are their corresponding video names?':
